...
class Model(ModelDesc):
   def __init__(self, depth, class_0, class_1, drop_rate, train_or_test):
      super(Model, self).__init__()
      #self.step = tf.train.get_or_create_global_step()
      self.N = int((depth - 4)  / 3)
      self.growthRate = 32
      self.filters_init = 32
   ...
   return [tf.TensorSpec((None, 224, 224, 3), tf.float32, 'input'),tf.TensorSpec((None,), tf.int32, 'label')]"""
...
def get_data(train_or_test, shuffle = None, multi_crop = None, crop_per_case = None, unknown_dir = None, original_dir=None):
   ...
   if isTrain:
      ...
      #                        nr_thread= batch_size*args.num_gpu,
      #                        map_func = aug_map_func,
      #                        buffer_size=batch_size*args.num_gpu)
      #
...
def get_config(train_or_test, train_config = None):
   ...
   denseModel = Model(depth=args.depth, class_0 = args.class_0, class_1 = args.class_1, drop_rate = drop_rate, train_or_test=isTrain)
   print(">>> Done.")
      ...
      return TrainConfig(
         ...
         callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate',
                                      [(args.drop_0, args.num_gpu*args.lr_0), (args.drop_1,  args.num_gpu*args.lr_1), (args.drop_2,  args.num_gpu*args.lr_2), (args.drop_3,  args.num_gpu*args.lr_3)]) # denote current hyperparameters
         ],
         model=denseModel,
...
if __name__ == '__main__':
   ...
   if args.tot == 'train':
      ...
      data = get_data('test', multi_crop = args.multi_crop, crop_per_case = args.crop_per_case, shuffle = False,unknown_dir = args.unknown_dir, original_dir=args.original_dir)
      ...
      def classify_unknown(model_config, dp, args):
         ...
         if bool(args.multi_crop) != False:
            multi_crop_count = args.multi_crop
            crop_per_case = min(data.size()-1, args.crop_per_case)
         crop_count_init = multi_crop_count
         ...
         for i, op in enumerate(res[0][4]):
            ...
            multi_crop_count -= 1
            crop_per_case -= 1
            crop_preds_bl.append(float(res[0][1][i][0]))
            crop_preds_dl.append(float(res[0][1][i][1]))
            
            if multi_crop_count == 0 or crop_per_case <= 0:
               multi_crop_count = args.multi_crop
               
               crop_preds_bl = []
               crop_preds_dl = []
               
               pred_bl = str(sum(crop_preds_bl)/float(args.multi_crop))
               pred_dl = str(sum(crop_preds_dl)/float(args.multi_crop))
               ...
               prediction_file.write(pred_bl+','+pred_dl+"\n")
               ...
               multi_crop_count = crop_count_init
