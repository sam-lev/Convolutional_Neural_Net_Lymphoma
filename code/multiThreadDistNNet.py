...
class Model(ModelDesc):
   ...
   def _build_graph(self, input_vars):
      ...
      def add_transition(name, l, drop_rate, training, drop_pattern, transition_number):
         shape = l.get_shape().as_list()
         in_channel = shape[3]
         with tf.variable_scope(name) as scope:
      ...
      factorbl = tf.divide(tf.add(self.class_0, self.class_1), tf.multiply(tf.constant(2.0,dtype=tf.float32), self.class_0))
      factordl = tf.divide(tf.add(self.class_0, self.class_1), tf.multiply(tf.constant(2.0,dtype=tf.float32), self.class_1))
...
def get_data(train_or_test, shuffle = None, image_size = None, scale_size = None, scale = None, multi_crop = None, crop_per_case = None, normalize = None, unknown_dir = None, original_dir=None):
   ...
   ds = datapack.lymphoma2(train_or_test, image_size = image_size, scale_size = scale_size, scale = scale, multi_crop=multi_crop, crop_per_case = crop_per_case, normalize = normalize, shuffle = shuffle, dir = '../data', unknown_dir = unknown_dir,original_dir=original_dir)
