...
class Model(ModelDesc):
    def __init__(self, depth, class_0, class_1, drop_rate, train_or_test):
        ...
        self.growthRate = 24
        self.filters_init = 24
        self.drop_rate = drop_rate
        self.training = train_or_test
        if class_0 == class_1:
           self.class_0 = 1.0
           self.class_1 = 1.0
        else:
    ...
    def _build_graph(self, input_vars):
        ...
        def dense_net(name):
            ...
            with tf.variable_scope('block1') as scope:
                ...
                l = add_transition('transition1', l ,self.drop_rate, self.training)
            
            with tf.variable_scope('block2') as scope:
            
                for i in range(self.N):
                    l = add_layer('dense_layer.{}'.format(i), l)
                l = add_transition('transition2', l, self.drop_rate, self.training)
...
def get_data(train_or_test, unknown_dir = None, original_dir=None):
    ...
    if isTrain:
       ...
       aug_map_func=lambda dp: [augmentor._augment(dp[0], param=((0.7, 1.3, np.random.randint(2**32-1)),
                                                             (0),
                                                             (True, False, 0.5))),dp[1]]
    else:
        augmentors = [
        ...
        #aug_map_func=lambda dp: [augmentor._augment(dp[0], param=((0.7, 1.3, np.random.randint(2**32-1)),(0))),dp[1]]
